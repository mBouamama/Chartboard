...
TIPBOARD_URL = PROPERTIES['TIPBOARD_URL'] if 'TIPBOARD_URL' in PROPERTIES else "http://0.0.0.0:8080/api"  # default url
REDIS_HOST = PROPERTIES['REDIS_HOST'] if 'REDIS_HOST' in PROPERTIES else "0.0.0.0"  # default redis host
REDIS_PORT = PROPERTIES['REDIS_PORT'] if 'REDIS_PORT' in PROPERTIES else 6379  # default redis port
REDIS_PASSWORD = PROPERTIES['REDIS_PASSWORD'] if 'REDIS_PASSWORD' in PROPERTIES else None  # default redis password
REDIS_DB = PROPERTIES['REDIS_DB'] if 'REDIS_DB' in PROPERTIES else 0  # default redis database
API_KEY = PROPERTIES['TIPBOARD_TOKEN'] if 'TIPBOARD_TOKEN' in PROPERTIES else get_random_secret_key() # default token create a random key by default
DEBUG = PROPERTIES['DEBUG'] if 'DEBUG' in PROPERTIES else True
LOG = PROPERTIES['LOG'] if 'LOG' in PROPERTIES else True
